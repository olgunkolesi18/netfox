...
struct TriangularFirstAlert: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 0) {
                KFImage(URL(string: model?.firstAlert?.icon ?? ""))
                    .resizable()
                    .scaledToFit()
                    .frame(width: 40, height: 40)
                
                VStack(spacing: 6) {
                    Text(model?.firstAlert?.title ?? "")
                        .font(.system(size: 17, weight: .semibold))
                        .foregroundColor(.black)
                        .multilineTextAlignment(.center)
                    
                    Text(model?.firstAlert?.subtitle ?? "")
                ...
                Button(model?.firstAlert?.button ?? "") {
                    onButtonTapped()
                    dismissAlert()
                }
                .font(.system(size: 17, weight: .semibold))
